...
def _generate_summary(mode: str, full_text: str):
    ...
    _ENTITY_RULES = (
        "具體實體擷取規則（最重要）：\n"
        "- 日期／期限：若會議中出現多次修改，只記錄最後確認的結果，不列過程中的舊版本。\n"
        "- 金額：只記錄最終核准的總金額，不列原始預算、差額或分項款項，除非這些是獨立決議。\n"
        "- 人名：只保留直接與會並參與決策的人員，不列閒聊中提及的第三方（如維修人員、供應商聯絡人）。\n"
        "- 地點：只保留最終確認的執行地點，排除被否決或替換的備選地點。\n"
        "- 事件：聚焦於會議最終決議的行動，排除閒聊話題（如餐食安排、體育賽事）及純技術討論細節。"
    )
    system_prompt = ""
    if mode == "full":
        system_prompt = (
            "你是一位具備 10 年經驗的專業會議記錄與商業分析師。請根據提供的會議逐字稿，以台灣繁體中文撰寫一份精鍊、高資訊密度的摘要。全程必須使用繁體中文，嚴禁出現簡體字。\n\n"
            "【輸出格式】只輸出以下兩個段落，不得輸出其他標題或說明文字：\n"
            "會議背景：用一句話說明會議的主要目的或核心討論內容。\n"
            "關鍵內容：條列討論過程中的邏輯重點與最終決議。\n\n"
            "【指令】\n"
            f"{_ENTITY_RULES}\n"
            "忽略寒暄、閒聊（如餐食安排、體育話題）及與業務決策無關的重複性對話，不在輸出中提及這類過濾動作。\n"
            "僅限使用逐字稿內容，嚴禁推測與補充。保持客觀中立，不使用誇飾語句。"
        )
        ...
        system_prompt = (
            ...
            "每個重點必須包含 1 個以上的具體細節（如：由誰負責、截止日期、具體數據）。\n\n"
            f"{_ENTITY_RULES}\n\n"
            "【範例參考】\n"
            "• [決議] UI 設計稿定案，將於 3/20 由設計組提交最終版本。\n"
            "• [待辦] 工程師 A 需在下週三前完成 API 串接文件的初步審查。\n\n"
            "【嚴格規則】\n"
        ...
        system_prompt = (
            ...
            "【輸出格式】只輸出下列兩個區塊，不得輸出其他標題或說明文字：\n"
            "【全文摘要】\n"
            "會議背景：用一句話說明主要目的或核心討論內容。\n"
            "關鍵內容：條列邏輯重點與最終決議。\n\n"
            "【重點條列】\n"
            "以「•」開頭，產出 3-7 個行動導向重點，格式為「[分類] 描述」。\n"
            "分類標籤：[決議]、[進度]、[問題]、[待辦]。\n"
            "每點必須含具體細節（負責人、期限、數據）。\n\n"
            "【指令】\n"
            f"{_ENTITY_RULES}\n"
            "忽略寒暄、閒聊及與業務決策無關的對話，不在輸出中提及這類過濾動作。\n"
            "僅限使用逐字稿內容，嚴禁推測與補充。保持客觀中立，不使用誇飾語句。"
        )
